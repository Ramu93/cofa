...
switch($action){
    ...
    break;
    case 'update_shoot_status':
    	$finaloutput = update_shoot_status();
    	break;
    default:
...
function update_shoot_status(){
	global $dbc;
	$job_uuid = mysqli_real_escape_string($dbc, trim($_POST['job_uuid']));
	$shoot_status = mysqli_real_escape_string($dbc, trim($_POST['job_status']));
	$shootprev_status = mysqli_real_escape_string($dbc, trim($_POST['jobprev_status']));
	...
	$query2 = "UPDATE jobs SET job_status = '$shoot_status', remarks_shoot='$remarks' WHERE job_uuid = '$job_uuid'";
	if(mysqli_query($dbc, $query2)){
		$inputs = array('job_uuid'=>$job_uuid,'log_type'=>'shoot','status_from'=>$shootprev_status,'status_to'=>$shoot_status,'errmsg'=> 'Error while logging shoot - '.$shoot_status);
		...
		if($endpoint == 'shoot'){
			...
			if(mysqli_num_rows($result3)>0){
	                while($row = mysqli_fetch_assoc($result3)){
	                	// check if a job is completed. if any one of the job is not complete, the loop will break
	                	if($row['job_status'] == 'shootcompleted'){
		...
		$output = array("infocode" => "SHOOTUPDATED", "message" => "Shoot status updated successfully");
	}else{
		writeerrorlog("\n".date('d-m-Y H:i:s')." : Error updating shoot - {$shoot_status} : ".$inputs['job_uuid']);
		$output = array("infocode" => "SHOOTUPDATEFAILED", "message" => "Error occurred while updating shoot status");
	}
	return $output;
}
